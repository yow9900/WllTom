...
PREPEND_SILENCE_SEC=int(env.get("PREPEND_SILENCE_SEC","0"))
...
AUDIO_TARGET_FORMAT = env.get("AUDIO_TARGET_FORMAT", "flac")
...
def convert_to_audio_format(input_path,output_path):
    run_ffmpeg(["-y","-i",input_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),output_path])

def get_audio_duration(path):
    if FFPROBE_BINARY:
        try:
            out=subprocess.check_output([FFPROBE_BINARY,"-v","error","-show_entries","format=duration","-of","default=noprint_wrappers=1:nokey=1",path],stderr=subprocess.DEVNULL)
            return float(out.strip())
        except:
            pass
    if path.lower().endswith(".wav"):
        with wave.open(path,'rb') as wf: return wf.getnframes()/float(wf.getframerate())
    tmp=tempfile.NamedTemporaryFile(delete=False,suffix=".wav"); tmp.close()
    try:
        run_ffmpeg(["-y","-i",path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),tmp.name])
        with wave.open(tmp.name,'rb') as wf: dur=wf.getnframes()/float(wf.getframerate())
    finally:
...
def split_wav_to_chunks_with_overlap(audio_path,out_dir,chunk_duration_sec,overlap_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    ...
    for i in range(num):
        ...
        subprocess.run([FFMPEG_BINARY,"-y","-ss",str(start),"-t",str(chunk_duration_sec),"-i",audio_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),out_name],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
...
def transcribe_file_with_speech_recognition(input_file_path,language_code):
    ...
    try:
        base_converted=os.path.join(tmpdir,f"converted.{AUDIO_TARGET_FORMAT}")
        try: convert_to_audio_format(input_file_path,base_converted)
        except Exception as e: raise RuntimeError("Conversion to "+AUDIO_TARGET_FORMAT.upper()+" failed: "+str(e))
        chunk_files=split_wav_to_chunks_with_overlap(base_converted,tmpdir,CHUNK_DURATION_SEC,CHUNK_OVERLAP_SEC)
