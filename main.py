...
# Waa loo beddelay 16000 Hz (16 kHz) si uu u helo saxnaanta ugu sarreysa ee Google ASR.
AUDIO_SAMPLE_RATE=int(env.get("AUDIO_SAMPLE_RATE","16000"))
AUDIO_CHANNELS=int(env.get("AUDIO_CHANNELS","1"))

# PREPEND_SILENCE_SEC: Waxaa loo yareeyay 0.3 sekan si loo yareeyo dib u dhaca laakiin aan la goyn hadalka.
PREPEND_SILENCE_SEC=float(env.get("PREPEND_SILENCE_SEC","0.3"))

# AMBIENT_CALIB_SEC: Waxaa loo kordhiyey 1.0 sekan si loo hubiyo in si sax ah loo cabbiray buuqa deegaanka.
AMBIENT_CALIB_SEC=float(env.get("AMBIENT_CALIB_SEC","1.0"))

# NEW: U ogolow isticmaalaha inuu beddelo format-ka loo beddelayo kahor inta aan loo dirin speech_recognition. (WAV waa midda ugu wanaagsan).
OUTPUT_FORMAT=env.get("OUTPUT_FORMAT","wav").lower()
if OUTPUT_FORMAT not in ["wav","flac","ogg"]: OUTPUT_FORMAT="wav"
...
if not SECRET_KEY:
    logging.critical("SECRET_KEY environment variable is required")
...
# Waxaa loo beddelay si uu u isticmaalo OUTPUT_FORMAT cusub
def convert_audio_to_base_format(input_path,output_path):
    # FFMPEG wuxuu go'aaminayaa format-ka kadib extension-ka
    run_ffmpeg(["-y","-i",input_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),output_path])

# Waxaa loo beddelay si uu u isticmaalo OUTPUT_FORMAT cusub
def get_base_format_duration(file_path):
    if OUTPUT_FORMAT=="wav":
        with wave.open(file_path,'rb') as wf: return wf.getnframes()/float(wf.getframerate())
    # Haddii aanu ahayn WAV, waxa loo baahan yahay in lagu ogaado FFMPEG.
    # Waa xalka fudud maxaa yeelay 'speech_recognition' ma bixiso tan.
    try:
        result = subprocess.run(
            [FFMPEG_BINARY, '-i', file_path, '-f', 'null', '-'],
        ...
        raise RuntimeError("Could not determine audio duration using ffprobe/ffmpeg.")
    except Exception as e:
        logging.error(f"Error determining duration for {file_path}: {e}")
        # Haddii uu jiro qalad, waxaan ku qasbeynaa 59 sekan si looga hortago dib u dhac
        return CHUNK_DURATION_SEC 

def prepend_silence_to_base_format(original_file,output_file,silence_sec):
    tmp_dir=os.path.dirname(output_file) or tempfile.gettempdir()
    silence_file=os.path.join(tmp_dir,f"silence_{int(time.time()*1000)}.{OUTPUT_FORMAT}")
    
    # OUTPUT_FORMAT=flac: Isticmaal commandkan si uu u sameeyo flac
    # OUTPUT_FORMAT=wav: Isticmaal commandkan si uu u sameeyo wav
    ...
    # Isku darida silence iyo original file
    run_ffmpeg(["-y","-i",silence_file,"-i",original_file,"-filter_complex","[0:0][1:0]concat=n=2:v=0:a=1[out]","-map","[out]",output_file])
    try: os.remove(silence_file)
    except: pass

def split_base_format_to_chunks_with_overlap(base_format_path,out_dir,chunk_duration_sec,overlap_sec):
    if FFMPEG_BINARY is None: raise RuntimeError("ffmpeg binary not found")
    if overlap_sec>=chunk_duration_sec: raise ValueError("overlap_sec must be smaller than chunk_duration_sec")
    os.makedirs(out_dir,exist_ok=True)
    total_dur=get_base_format_duration(base_format_path) # Hadda wuxuu isticmaalayaa shaqada cusub
    ...
    for i in range(num):
        ...
        out_name=os.path.join(out_dir,f"chunk{i:03d}.{OUTPUT_FORMAT}") # Isticmaal OUTPUT_FORMAT
        subprocess.run([FFMPEG_BINARY,"-y","-ss",str(start),"-t",str(chunk_duration_sec),"-i",base_format_path,"-ar",str(AUDIO_SAMPLE_RATE),"-ac",str(AUDIO_CHANNELS),out_name],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
...
def create_prepended_chunk(chunk_path,silence_sec):
    tmp=tempfile.NamedTemporaryFile(delete=False,suffix=f".{OUTPUT_FORMAT}"); tmp.close() # Isticmaal OUTPUT_FORMAT
    try:
        prepend_silence_to_base_format(chunk_path,tmp.name,silence_sec); return tmp.name # Isticmaal shaqada cusub
    except:
...
def transcribe_file_with_speech_recognition(input_file_path,language_code):
    ...
    try:
        base_file=os.path.join(tmpdir,f"converted.{OUTPUT_FORMAT}") # Isticmaal OUTPUT_FORMAT
        try: convert_audio_to_base_format(input_file_path,base_file) # Isticmaal shaqada cusub
        except Exception as e: raise RuntimeError(f"Conversion to {OUTPUT_FORMAT.upper()} failed: "+str(e))
        chunk_files=split_base_format_to_chunks_with_overlap(base_file,tmpdir,CHUNK_DURATION_SEC,CHUNK_OVERLAP_SEC) # Isticmaal shaqada cusub
