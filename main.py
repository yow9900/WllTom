...
def assembly_upload_file(file_path):
...
def assembly_create_transcript(audio_url):
    if not ASSEMBLYAI_API_KEY: raise RuntimeError("ASSEMBLYAI_API_KEY not set")
    headers={"authorization":ASSEMBLYAI_API_KEY,"content-type":"application/json"}
    payload={"audio_url":audio_url,"language_detection":True,"format_text":True,"punctuate":True}
    resp=requests.post("https://api.assemblyai.com/v2/transcript",headers=headers,json=payload,timeout=REQUEST_TIMEOUT_GEMINI)
    resp.raise_for_status()
    return resp.json().get("id")

def assembly_get_transcript(transcript_id):
    headers={"authorization":ASSEMBLYAI_API_KEY}
    resp=requests.get(f"https://api.assemblyai.com/v2/transcript/{transcript_id}",headers=headers,timeout=REQUEST_TIMEOUT_GEMINI)
    resp.raise_for_status()
    return resp.json()
...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    try:
        ...
        try:
            upload_url=assembly_upload_file(tmpf_name)
            if not upload_url: raise RuntimeError("Upload failed")
            transcript_id=assembly_create_transcript(upload_url)
            if not transcript_id: raise RuntimeError("Transcription creation failed")
            start_time=time.time()
            text=None
            while True:
                info=assembly_get_transcript(transcript_id)
                status=info.get("status")
                if status=="completed":
                    text=info.get("text","")
                    break
                if status=="error":
                    raise RuntimeError(info.get("error","Transcription failed"))
                if time.time()-start_time>REQUEST_TIMEOUT_GEMINI:
    ...
    while True:
        try:
            ...
            if item:
                ...
                logging.info(f"Queue process user {message.from_user.id}")
...
def handle_media_common(message,bot_obj,bot_token,bot_index=0):
    ...
    if file_size and file_size>int(env.get("TELEGRAM_MAX_BYTES",str(20*1024*1024))):
        max_display_mb=int(env.get("TELEGRAM_MAX_BYTES",str(20*1024*1024)))//(1024*1024)
