...
from assemblyai import Client as AssemblyAIClient, TranscribeConfig
...
if not ASSEMBLYAI_API_KEY:
    logging.critical("ASSEMBLYAI_API_KEY environment variable is required")
...
aa_client=AssemblyAIClient(api_key=ASSEMBLYAI_API_KEY)
...
def transcribe_via_assemblyai(file_url,lang_code):
    try:
        config=TranscribeConfig(language_code=lang_code)
        transcript=aa_client.transcribe_url(file_url,config)
        if transcript.status=='error': raise RuntimeError(transcript.error or "AssemblyAI transcription error")
        return transcript.text or "","assemblyai"
...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    try:
        ...
        try:
            ...
            bot_obj.send_message(message.chat.id,f"⚠️ Transcription error: {error_msg}",reply_to_message_id=message.message_id)
            return
