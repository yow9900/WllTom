...
env = os.environ
ASSEMBLYAI_API_KEY = env.get("ASSEMBLYAI_API_KEY", "")
...
REQUEST_TIMEOUT_ASSEMBLYAI = int(env.get("REQUEST_TIMEOUT_ASSEMBLYAI", "300"))
...
action_usage = {}
ALLOWED_EXTENSIONS = set(["mp3", "wav", "m4a", "ogg", "webm", "flac", "mp4", "mkv", "avi", "mov", "aac", "aiff", "amr", "wma", "opus", "m4v", "ts", "flv", "3gp"])
...
def upload_to_assemblyai(audio_url):
    ...
    response = requests.post("https://api.assemblyai.com/v2/transcript", json=data, headers=headers, timeout=REQUEST_TIMEOUT_ASSEMBLYAI)
    response.raise_for_status()
    return response.json()["id"]

def get_assemblyai_transcription(transcript_id):
    headers = {"authorization": ASSEMBLYAI_API_KEY}
    while True:
        response = requests.get(f"https://api.assemblyai.com/v2/transcript/{transcript_id}", headers=headers, timeout=REQUEST_TIMEOUT_ASSEMBLYAI)
        response.raise_for_status()
        ...
        if status == "error":
            raise RuntimeError(f"AssemblyAI transcription failed: {transcript_data.get('error', 'Unknown error')}")
        time.sleep(3)

def transcribe_via_assemblyai(audio_url, lang_code):
    try:
        transcript_id = upload_to_assemblyai(audio_url)
        text = get_assemblyai_transcription(transcript_id)
        if not text:
...
def process_media_file(message, bot_obj, bot_token, bot_index, file_id, file_size, filename):
    ...
    try:
        ...
        text, used_service = transcribe_via_assemblyai(file_url, lang)
        corrected_text = normalize_text_offline(text)
        uid_key = str(message.chat.id)
        user_mode = get_user_send_mode(uid_key)
        if not corrected_text:
            bot_obj.send_message(message.chat.id, "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_to_message_id=message.message_id)
            return
        if len(corrected_text) > 4000:
            ...
            sent_msg = bot_obj.send_message(message.chat.id, corrected_text, reply_to_message_id=message.message_id)
        ...
        try:
            bot_obj.send_message(message.chat.id, f"Error during transcription: {str(e)}", reply_to_message_id=message.message_id)
        except:
            pass
    finally:
        stop["stop"] = True
...
def worker_thread():
    while True:
        ...
        try:
            ...
            logging.info(f"Processing queued media from user {message.from_user.id} chat {message.chat.id}")
            process_media_file(message, bot_obj, bot_token, bot_index, file_id, file_size, filename)
        except:
            logging.exception("Error in worker thread processing item")
        finally:
...
def ask_gemini(text, instruction, timeout=REQUEST_TIMEOUT_ASSEMBLYAI):
